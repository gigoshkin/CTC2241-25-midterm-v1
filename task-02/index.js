...
function calcAverage(grades) {
	const gradesArr = Object.values(grades);
	return gradesArr.reduce((a, b) => a + b) / gradesArr.length;
}
...
function getHighAchievers(students, gradeThreshold) {
	return students.filter((student) => {
		return calcAverage(student.grades) > gradeThreshold;
	}).map((student) => {
...
function getMultiAchivers(students, threshold) {
	const res = students.map((student) => {
		let gradesArr = Object.values(student.grades);
		gradesArr = gradesArr.filter((grade) => grade > threshold)
		return {
			name: getName(student),
			expertSubjects: gradesArr.length
		};
	});

	return res.filter((student) => student.expertSubjects >= 2);
}
